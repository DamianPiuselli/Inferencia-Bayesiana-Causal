...
def _1_1(
    enunciado="""Todas las opiniones son válidas porque no hay una forma correcta, universal, de evaluar las diferentes opiniones alternativas.""",
):

    respuestas = {}

    # Verdadero, no hay una forma correcta de evaluar las diferentes opiniones alternativas
    respuestas["Verdadero"] = 0.01

    # Falseo, sí existe una forma correcta de evaluar las diferentes opiniones alternativas
    respuestas["Falso"] = 0.99
...
def _1_2(
    enunciado="""Hay tres cajas idénticas. Detrás de una de ellas hay un regalo. El resto están vacías. ¿Dónde está el regalo?""",
):
    respuestas = {}
...
def _1_3(
    enunciado="""La ciencia tiene pretensión de alcanzar verdades, es decir, proposiciones que valgan para todas las personas más allá de sus subjetividades individuales. Las ciencias puramente formales, como la matemática, validan sus proposiciones derivando teoremas en sistemas axiomáticos cerrados sin incertidumbre. Las ciencias con datos, desde la física hasta las ciencias sociales, deben validar sus proposiciones en sistemas naturales abiertos que contienen siempre regiones ocultas a nuestra percepción. ¿Es posible determinar el valor de verdad de una proposición en contextos de incertidumbre si justamente tenemos incertidumbre sobre su estado real?""",
):
...
def _1_4(
    ...
    respuestas["Regalo = 1, Abren = 2"] = 1 / 6
    respuestas["Regalo = 1, Abren = 3"] = 1 / 6
    # El regalo está en la caja 2
    respuestas["Regalo = 2, Abren = 1"] = 1 / 6
    respuestas["Regalo = 2, Abren = 2"] = 0.0
    respuestas["Regalo = 2, Abren = 3"] = 1 / 6
    # El regalo está en la caja 3
    respuestas["Regalo = 3, Abren = 1"] = 1 / 6
    respuestas["Regalo = 3, Abren = 2"] = 1 / 6
...
def _1_5(
    enunciado="""Hay tres cajas idénticas. Detrás de una de ellas hay un regalo. El resto están vacías. Luego, una persona elige al azar una de las cajas que no contiene el regalo y nos muestra que el regalo no está en la caja 2. ¿Dónde está el regalo?""",
):
    nombre = (inspect.currentframe().f_code.co_name,)

    respuestas = {}
    # El regalo está en la caja 1
    respuestas["Regalo = 1"] = 1 / 2
    respuestas["Regalo = 2"] = 0.0
    respuestas["Regalo = 3"] = 1 / 2
...
def _1_9(
    enunciado="""En probabilidad, ¿cómo se predice un evento todavía no observado?""",
):
    ...
    # El regalo está en la caja 1 y reservamos caja 1
    respuestas[
        "Se ofrece la distribución de probabilidad marginal de la hipótesis a priori"
    ] = 0.1
    respuestas[
        "Se ofrece la distribución de probabilidad marginal de la hipótesis a posteriori"
    ] = 0.7
    respuestas[
        "Se propone la hipótesis que maximizó la predicción de los datos previamente observados"
    ] = 0.1
    respuestas[
        "Se proponen la hipótesis que maximiza la predicción de todos los datos observados"
    ] = 0.1
...
def _1_10(
    enunciado="""En probabilidad, ¿cómo se incorpora la información contenida en un nuevo dato?""",
):
    ...
    # El regalo está en la caja 1 y reservamos caja 1
    respuestas[
        "Se preserva la creencia previa que sigue siendo compatible con el dato"
    ] = 0.5
    respuestas[
        "Se entrena el modelo, seleccionando los parámetros que mejor ajustan a los datos"
    ] = 0.5
